<template>
  <div>
    <div class="row">
      <div class="col-md-12">
        <v-card>
          <v-card-title>Edit Laporan Kegiatan Organisasi Kemahasiswaan</v-card-title>
          <v-card-text>
...
export default {
  ...
  methods: {
    ...mapActions("organization", ["updateOrganization", "getOrganization"]),
    ...mapActions("semester", ["getSemesterList"]),
    async onFetchData() {
      await this.getOrganization({ id: this.id });
    },
    async validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.organizationData.document != null) {
