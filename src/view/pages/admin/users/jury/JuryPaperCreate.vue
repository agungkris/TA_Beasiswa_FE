  ...
  <div>
    <div class="row">
      <div class="col-md-12">
        <v-card class="mb-5">
          <v-card-title>
            <span class="title font-weight-light" style="padding-right:5px;">Pemilihan</span>
            <span class="title font-weight-light" style="padding-right:5px;">Karya</span>
            <span class="title font-weight-light" style="padding-right:5px;">Tulis</span>
            <span class="title font-weight-light">Mahasiswa</span>
          </v-card-title>
          <v-card-text>
            <v-form ref="form" v-model="valid" lazy-validation>
              <v-text-field v-model="createjuryData.name" :rules="nameRules" disabled></v-text-field>
              <v-card-text>
                <v-select class="my-2" label="Periode Pengajuan Beasiswa" target="#dropdown-example"
                  v-model="selectedPeriod" :items="periodList" item-value="id" @change="onChangeFilter"
                  item-text="name"></v-select>
                <v-data-table :items="uploadscholarshipList" :headers="headers" item-key="name" :search="search"
                  ...
                  <template v-slot:[`item.pilihan`]="{ item }">
                    <v-checkbox :value="item.id" v-model="paperjuryData.submission_id"></v-checkbox>
                  </template>
                </v-data-table>
              </v-card-text>
          ...
          <v-card-text>
            <v-data-table :headers="headers2" :items="paperjuryList">
              <template v-slot:[`item.action`]="{ item }">
                <v-btn color="red darken-4" text icon @click="onRemove(item.id)" class="mr-2">Hapus</v-btn>
              </template>
            </v-data-table>
          </v-card-text>
        </v-card>
