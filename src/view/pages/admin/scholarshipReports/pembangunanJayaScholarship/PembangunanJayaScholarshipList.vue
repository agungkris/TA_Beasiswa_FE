  ...
  <div>
    <v-card class="mb-6">
      <v-card-title>
        ...
        <span class="title font-weight-light" style="padding-right:5px;">Pendapat</span>
        ...
        <v-spacer></v-spacer>
        <v-select class="my-2" label="Periode Pengajuan Beasiswa" target="#dropdown-example" v-model="selectedPeriod"
          :items="periodList" item-value="id" @change="onChangeFilter" item-text="name"></v-select>
        <v-spacer></v-spacer>
        <v-text-field class="my-2" v-model="searchfinal" append-icon="mdi-magnify" label="Search" single-line
          hide-details></v-text-field>
      </v-card-title>
      <v-card-text>
        <v-data-table :headers="headers" :items="uploadscholarshipFgd" :search="searchfinal" :footer-props="{
